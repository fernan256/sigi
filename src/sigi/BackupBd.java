...
public class BackupBd extends javax.swing.JInternalFrame {
    ...
    private void restaurarBdActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_restaurarBdActionPerformed
            ...
            if(restoreRout.getText().length() > 0){
                try{
                    ...
                    int c = JOptionPane.showConfirmDialog(null, "Restaurar BD", "Confirmacion", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
                    if(c == JOptionPane.YES_OPTION){
                        ...
                        mysql = new Conexion();
                        backup = "cmd /c mysql --user=root --password=dedox132 sigi < " + path;
    ...
    private void createBackup () {
        ...
        int routeOk = JOptionPane.showOptionDialog(null, "Confirmar ruta: "+saveRoute.getText(), "Respaldo de Base de Datos", JOptionPane.PLAIN_MESSAGE, JOptionPane.WARNING_MESSAGE, null, options, options[0]);
        if(routeOk == 0){
            try{
                con = new Conexion();
